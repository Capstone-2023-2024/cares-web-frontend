...
import type { PermissionProps } from "@cares/types/permission"
      ...
      async function extractAccounts() {
        if (user !== null) {
          try {
            ...
            const data = await handler(user.email ?? "")
            if (data !== null) {
              const { role } = data
              switch (role.name) {
                case "super_admin":
                  return handleState(
                    "typeOfAccount",
                    role.partial ? "partial_super_admin" : "super_admin"
                  )
                case "admin":
                  return handleState(
                    "typeOfAccount",
                    role.partial ? "admin" : "program_chair"
                  )
                case "sub_admin":
                  return handleState(
                    "typeOfAccount",
                    role.partial ? "organization_president" : "board_member"
                  )
                // case "faculty":
